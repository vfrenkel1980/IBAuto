...
public class DownloadPageObject {
    ...
    /**@todo return xpath of terms after Andrey fix */
    //public static final By terms = By.xpath("//*[contains(text(),'You must approve IncrediBuild's end user license agreement')]");
    public static final By terms = By.xpath("//*[contains(text(),'You must approve IncrediBuild')]");
    ...
    public void updateUserInfo(UpdateInfoForm uif){
        ...
        wait.until(ExpectedConditions.visibilityOfElementLocated(UPDATE_FNAME)).clear();
        eventWebDriver.findElement(UPDATE_FNAME).sendKeys(uif.getName());
        eventWebDriver.findElement(UPDATE_LNAME).clear();
        eventWebDriver.findElement(UPDATE_LNAME).sendKeys(uif.getLname());
        SystemActions.sleep(2);
        eventWebDriver.findElement(UPDATE_PHONE).clear();
        eventWebDriver.findElement(UPDATE_PHONE).sendKeys(uif.getPhone());
        eventWebDriver.findElement(UPDATE_COMPANY).clear();
        eventWebDriver.findElement(UPDATE_COMPANY).sendKeys(uif.getCompany());
        eventWebDriver.findElement(UPDATE_CITY).clear();
        eventWebDriver.findElement(UPDATE_CITY).sendKeys(uif.getCity());
        wait.until(ExpectedConditions.visibilityOfElementLocated(COUNTRY_SELECTION_DDL));
        Select country = new Select(eventWebDriver.findElement(COUNTRY_SELECTION_DDL));
        country.selectByVisibleText(uif.getCountry());
        eventWebDriver.findElement(SUBMIT_BTN).click();
    }
    ...
    public void verifyUpdatedUserInfo(UpdateInfoForm uif){
        ...
        wait.until(ExpectedConditions.invisibilityOfElementLocated(THANK_YOU_LABEL));
        WebElement fName = eventWebDriver.findElement(UPDATE_FNAME);
